...
ipcMain.on('request-side-menu', (event, arg) => {
  const hasActiveTab = tabManager.hasActiveTab()
  const hasTab = tabManager.hasTabs();
  const sideMenu = sideMenuConstructor(hasActiveTab, hasTab)
  Menu.buildFromTemplate(sideMenu as any).popup(mainWindow);
});
...
function initMenu() {
  const constructedSideMenu = sideMenuConstructor(true, true)
  sideMenu = Menu.buildFromTemplate(constructedSideMenu as any);
