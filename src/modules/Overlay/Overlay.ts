...
class Overlay extends EventEmitter {
  ...
  constructor(window: BrowserWindow, appPath: string) {
    ...
    this.view.webContents.loadFile(
      this.appPath + "/pages/overlay/overlay.html"
    );
    ...
    this.view.webContents.on("context-menu", (event, params) => {
      if (params.isEditable) {
        const editMenu = Menu.buildFromTemplate([
          {
            label: "Cut",
            icon: this.appPath + "/assets/imgs/icons16/cut.png",
          ...
          {
            label: "Paste",
            icon: this.appPath + "/assets/imgs/icons16/paste.png",
          ...
          {
            label: "Undo",
            icon: this.appPath + "/assets/imgs/icons16/undo.png",
          ...
          {
            label: "Redo",
            icon: this.appPath + "/assets/imgs/icons16/redo.png",
          ...
          {
            label: "Select all",
            icon: this.appPath + "/assets/imgs/icons16/select-all.png",
          ...
          {
            label: "Delete",
            icon: this.appPath + "/assets/imgs/icons16/delete.png",
  ...
  showButtonMenu(): void {
    const buttonMenu = Menu.buildFromTemplate([
      {
        label: "Show overlay",
        icon: this.appPath + "/assets/imgs/icons16/overlay.png",
        accelerator: "F1",
        click: (): void => {
          this.show();
        },
      ...
      {
        label: "Bookmarks",
        icon: this.appPath + "/assets/imgs/icons16/bookmarks.png",
      ...
      {
        label: "History",
        icon: this.appPath + "/assets/imgs/icons16/history.png",
      ...
      {
        label: "Downloads",
        icon: this.appPath + "/assets/imgs/icons16/download.png",
  ...
  addFolderWithBookmarks(
    folderName: string,
    bookmarks: { name: string; url: string; [key: string]: any }[]
  ): void {
  ...
  setDownloadStatusPause(download: IDownload): void {
    this.view.webContents.send(
      "downloadManager-setDownloadStatusPause",
      download
    );
  }
  ...
  setDownloadStatusDone(download: IDownload): void {
    this.view.webContents.send(
      "downloadManager-setDownloadStatusDone",
      download
    );
  }
  ...
  setDownloadStatusFailed(download: IDownload): void {
    this.view.webContents.send(
      "downloadManager-setDownloadStatusFailed",
      download
    );
  }
